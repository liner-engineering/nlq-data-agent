...
"""
    ...
    def _build_correction_feedback(
        ...
        # 비용 초과 에러 — 구체적 가이드 (5가지 최적화 규칙)
        if "bytes billed" in error.lower() or "1TB" in error or "데이터" in error.lower():
            parts.extend([
                "**BigQuery 비용 초과 해결 (5가지 최적화 규칙):**",
                "",
                "규칙 1) EVENTS_296805 파티션 필터 (필수!):",
                "  - 반드시: WHERE DATE(event_time) BETWEEN '2026-04-01' AND '2026-04-30'",
                "  - 금지: 시간 범위 없이 전체 스캔",
                "",
                "규칙 2) agent_credit_usage_log 조기 필터 (필수!):",
                "  - 반드시: WHERE acu.user_id IN (SELECT ... FROM target_users)  -- 좁은 범위",
                "  - 반드시: AND DATE(acu.created_at) BETWEEN '...' AND '...'",
                "  - 금지: 100M 행 테이블을 필터 없이 전체 스캔",
                "",
                "규칙 3) 같은 테이블 중복 읽기 금지:",
                "  - 금지: UNION ALL로 EVENTS_296805 여러 번 읽기",
                "  - 권장: 좁은 범위에서 IN절로 통합 (1번만 읽기)",
                "",
                "규칙 4) 좁은 세트 먼저, 큰 테이블은 나중에:",
                "  - 패턴: WITH target_users AS (좁은 필터) ... FROM large_table JOIN target_users",
                "  - 예시: EVENTS_296805에서 100명 추출 → agent_credit_usage_log는 100명만 조인",
                "",
                "규칙 5) CTE 설계: WHERE 조기 필터링, SELECT는 필요 컬럼만:",
                "  - 조기 필터: DATE(event_time) → event_type → liner_product 순서로",
                "  - SELECT에서 불필요한 컬럼 제거 (SELECT * 금지)",
                "",
                "예시 패턴:",
                "  WITH base AS (",
                "    SELECT user_id, liner_product",
                "    FROM EVENTS_296805",
                "    WHERE DATE(event_time) BETWEEN '2026-04-01' AND '2026-04-30'",
                "      AND event_type = 'make_chat'",
                "  )",
                "  SELECT ... FROM agent_credit_usage_log acu",
                "  WHERE acu.user_id IN (SELECT user_id FROM base WHERE liner_product = 'write')",
